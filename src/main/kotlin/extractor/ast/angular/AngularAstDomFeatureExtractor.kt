...
class AngularAstDomFeatureExtractor(
    private val command: String = System.getenv("UI_SIMILARITY_ANGULAR_DOM_AST_CMD")?.takeIf { it.isNotBlank() }
        ?: "node scripts/angular-dom-ast-extract.mjs",
    private val timeoutSeconds: Long = 10L,
    private val fallback: DomFeatureExtractor = SimpleDomFeatureExtractor(),
    private val onFailure: ((AngularDomAstFailureEvent) -> Unit)? = null
) : DomFeatureExtractor {
    ...
    override fun extractDomFeatures(source: ComponentSource): DomFeatures {
        ...
        if (output == null) {
            onFailure?.invoke(AngularDomAstFailureEvent(source.id, commandReason ?: "command_failure", fallbackUsed = true))
            return fallback.extractDomFeatures(source)
        }

        val (parsed, parseReason) = parseResponse(output)
        if (parsed == null) {
            onFailure?.invoke(AngularDomAstFailureEvent(source.id, parseReason ?: "parse_failure", fallbackUsed = true))
            return fallback.extractDomFeatures(source)
        }
...
data class AngularDomAstFailureEvent(
