...
class AngularAstCssFeatureExtractor(
    private val command: String = System.getenv("UI_SIMILARITY_ANGULAR_CSS_AST_CMD")?.takeIf { it.isNotBlank() }
        ?: "node scripts/angular-css-ast-extract.mjs",
    private val timeoutSeconds: Long = 10L,
    private val fallback: CssFeatureExtractor = SimpleCssFeatureExtractor(),
    private val onFailure: ((AngularCssAstFailureEvent) -> Unit)? = null
) : CssFeatureExtractor {
    ...
    override fun extractCssFeatures(source: ComponentSource): CssFeatures {
        ...
        if (output == null) {
            onFailure?.invoke(AngularCssAstFailureEvent(source.id, commandReason ?: "command_failure", fallbackUsed = true))
            return fallback.extractCssFeatures(source)
        }
        val (parsed, parseReason) = parseResponse(output)
        if (parsed == null) {
            onFailure?.invoke(AngularCssAstFailureEvent(source.id, parseReason ?: "parse_failure", fallbackUsed = true))
            return fallback.extractCssFeatures(source)
        }
...
data class AngularCssAstFailureEvent(
