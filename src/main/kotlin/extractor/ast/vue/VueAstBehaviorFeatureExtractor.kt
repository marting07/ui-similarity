...
class VueAstBehaviorFeatureExtractor(
    private val command: String = System.getenv("UI_SIMILARITY_VUE_BEHAVIOR_AST_CMD")?.takeIf { it.isNotBlank() }
        ?: "node scripts/vue-behavior-ast-extract.mjs",
    private val timeoutSeconds: Long = 10L,
    private val fallback: BehaviorFeatureExtractor = SimpleBehaviorFeatureExtractor(),
    private val onFailure: ((VueBehaviorAstFailureEvent) -> Unit)? = null
    ...
    override fun extractBehaviorFeatures(source: ComponentSource): BehaviorFeatures {
        ...
        if (output == null) {
            onFailure?.invoke(VueBehaviorAstFailureEvent(source.id, commandReason ?: "command_failure", fallbackUsed = true))
            return fallback.extractBehaviorFeatures(source)
        }
        val (parsed, parseReason) = parseResponse(output)
        if (parsed == null) {
            onFailure?.invoke(VueBehaviorAstFailureEvent(source.id, parseReason ?: "parse_failure", fallbackUsed = true))
            return fallback.extractBehaviorFeatures(source)
        }
...
data class VueBehaviorAstFailureEvent(
