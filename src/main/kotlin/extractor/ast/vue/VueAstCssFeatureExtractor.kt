...
class VueAstCssFeatureExtractor(
    private val command: String = System.getenv("UI_SIMILARITY_VUE_CSS_AST_CMD")?.takeIf { it.isNotBlank() }
        ?: "node scripts/vue-css-ast-extract.mjs",
    private val timeoutSeconds: Long = 10L,
    private val fallback: CssFeatureExtractor = SimpleCssFeatureExtractor(),
    private val onFailure: ((VueCssAstFailureEvent) -> Unit)? = null
) : CssFeatureExtractor {
    ...
    override fun extractCssFeatures(source: ComponentSource): CssFeatures {
        ...
        if (output == null) {
            onFailure?.invoke(VueCssAstFailureEvent(source.id, commandReason ?: "command_failure", fallbackUsed = true))
            return fallback.extractCssFeatures(source)
        }
        val (parsed, parseReason) = parseResponse(output)
        if (parsed == null) {
            onFailure?.invoke(VueCssAstFailureEvent(source.id, parseReason ?: "parse_failure", fallbackUsed = true))
            return fallback.extractCssFeatures(source)
        }
...
data class VueCssAstFailureEvent(
