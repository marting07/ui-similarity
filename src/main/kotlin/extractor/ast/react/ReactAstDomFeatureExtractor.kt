...
/**
 * React-focused DOM extractor backed by an external AST command.
 *
 * Falls back to [SimpleDomFeatureExtractor] for non-React sources or
 * when AST execution/parsing fails.
 */
class ReactAstDomFeatureExtractor(
    private val command: String = System.getenv("UI_SIMILARITY_REACT_DOM_AST_CMD")?.takeIf { it.isNotBlank() }
        ?: "node scripts/react-dom-ast-extract.mjs",
    private val timeoutSeconds: Long = 10L,
    private val fallback: DomFeatureExtractor = SimpleDomFeatureExtractor(),
    private val onFailure: ((ReactDomAstFailureEvent) -> Unit)? = null
) : DomFeatureExtractor {
    ...
    override fun extractDomFeatures(source: ComponentSource): DomFeatures {
        ...
        val payload = "{\"sourceCode\":\"${escape(source.templateCode)}\",\"styleCode\":\"${escape(source.styleCode)}\"}"
        val (output, commandReason) = runCommand(payload)
        if (output == null) {
            onFailure?.invoke(ReactDomAstFailureEvent(source.id, commandReason ?: "command_failure", fallbackUsed = true))
            return fallback.extractDomFeatures(source)
        }
        val (parsed, parseReason) = parseResponse(output)
        if (parsed == null) {
            onFailure?.invoke(ReactDomAstFailureEvent(source.id, parseReason ?: "parse_failure", fallbackUsed = true))
            return fallback.extractDomFeatures(source)
        }
...
data class ReactDomAstFailureEvent(
