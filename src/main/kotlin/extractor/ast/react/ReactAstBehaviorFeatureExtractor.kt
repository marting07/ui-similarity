...
class ReactAstBehaviorFeatureExtractor(
    private val command: String = System.getenv("UI_SIMILARITY_REACT_BEHAVIOR_AST_CMD")?.takeIf { it.isNotBlank() }
        ?: "node scripts/react-behavior-ast-extract.mjs",
    private val timeoutSeconds: Long = 10L,
    private val fallback: BehaviorFeatureExtractor = SimpleBehaviorFeatureExtractor(),
    private val onFailure: ((ReactBehaviorAstFailureEvent) -> Unit)? = null
    ...
    override fun extractBehaviorFeatures(source: ComponentSource): BehaviorFeatures {
        ...
        if (output == null) {
            onFailure?.invoke(ReactBehaviorAstFailureEvent(source.id, commandReason ?: "command_failure", fallbackUsed = true))
            return fallback.extractBehaviorFeatures(source)
        }
        val (parsed, parseReason) = parseResponse(output)
        if (parsed == null) {
            onFailure?.invoke(ReactBehaviorAstFailureEvent(source.id, parseReason ?: "parse_failure", fallbackUsed = true))
            return fallback.extractBehaviorFeatures(source)
        }
...
data class ReactBehaviorAstFailureEvent(
