...
object AngularAstCssExtractorTests {
    fun run(): List<Pair<String, Throwable?>> {
        return listOf(
            TestSupport.test("angular AST CSS extractor extracts tokens palette spacing and font buckets") {
                val extractor = AngularAstCssFeatureExtractor(command = "node scripts/angular-css-ast-extract.mjs")
            ...
            TestSupport.test("angular AST CSS extractor falls back when command fails") {
                val failures = mutableListOf<String>()
                val extractor = AngularAstCssFeatureExtractor(
                    command = "exit 1",
                    onFailure = { failures += it.reason }
                )
                val source = ComponentSource(
                    id = "angular#css-fallback",
                    framework = UiFramework.ANGULAR,
                    templateCode = "",
